...
///////////For Blue Led and buttons////////////////////////
#define BOARD_LEDB_GPIO     BOARD_LED_BLUE_GPIO
#define BOARD_LEDB_GPIO_PIN BOARD_LED_BLUE_GPIO_PIN

#define BOARD_SW_GPIO3        BOARD_SW3_GPIO
#define BOARD_SW_PORT3        BOARD_SW3_PORT
#define BOARD_SW_GPIO3_PIN    BOARD_SW3_GPIO_PIN
#define BOARD_SW_IRQ3         BOARD_SW3_IRQ
#define BOARD_SW_IRQ3_HANDLER BOARD_SW3_IRQ_HANDLER
#define BOARD_SW_NAME3        BOARD_SW3_NAME
...
//{
//		choose_topic,water_the_plants, uv_light_toggle, display_humidity
//};
...
volatile bool uvlight_led =false;
/*!
 * @brief Interrupt service fuction of switch.
 *

 */
void BOARD_SW_IRQ3_HANDLER(void)
{
#if (defined(FSL_FEATURE_PORT_HAS_NO_INTERRUPT) && FSL_FEATURE_PORT_HAS_NO_INTERRUPT)
    /* Clear external interrupt flag. */
    GPIO_GpioClearInterruptFlags(BOARD_SW_GPIO3, 1U << BOARD_SW_GPIO3_PIN);
#else
    /* Clear external interrupt flag. */
    GPIO_PortClearInterruptFlags(BOARD_SW_GPIO3, 1U << BOARD_SW_GPIO3_PIN);
#endif
...
static void app_thread(void *arg)
{
    ...
    if (sys_thread_new("main", check_button, NULL, INIT_THREAD_STACKSIZE, INIT_THREAD_PRIO) == NULL)
        {
            LWIP_ASSERT("main(): Task creation failed.", 0);
        }


//    sys_thread_new("w_event", check_button, NULL, APP_THREAD_STACKSIZE, APP_THREAD_PRIO);
//    if (sys_thread_new("w_event", check_button, NULL, INIT_THREAD_STACKSIZE, INIT_THREAD_PRIO) == NULL)
//        {
//            LWIP_ASSERT("main(): Task creation failed.", 0);
//        }
...
int main(void)
{
    ...
    #if (defined(FSL_FEATURE_PORT_HAS_NO_INTERRUPT) && FSL_FEATURE_PORT_HAS_NO_INTERRUPT)
        GPIO_SetPinInterruptConfig(BOARD_SW_GPIO3, BOARD_SW_GPIO3_PIN, kGPIO_InterruptFallingEdge);
    #else
        PORT_SetPinInterruptConfig(BOARD_SW_PORT3, BOARD_SW_GPIO3_PIN, kPORT_InterruptFallingEdge);
    #endif
        EnableIRQ(BOARD_SW_IRQ3);
        GPIO_PinInit(BOARD_SW_GPIO3, BOARD_SW_GPIO3_PIN, &sw_config);
...
void water_the_plants()
{
	static const char *topic   = "water/#";
	static const char *message = "plants were watered";

	mqtt_publish(mqtt_client, topic, message, strlen(message), 1, 0, mqtt_message_published_cb, (void *)topic);
}
void uv_light_toggle()
{
	static const char *topic   = "uvlight/#";
	static const char *message = "UV light led was toggled";
	GPIO_PortToggle(BOARD_LEDB_GPIO, 1U << BOARD_LEDB_GPIO_PIN);
	mqtt_publish(mqtt_client, topic, message, strlen(message), 1, 0, mqtt_message_published_cb, (void *)topic);
}
void display_humidity()
{
	static const char *topic   = "humidity/#";
	static const char *message = "80% humidity";
	mqtt_publish(mqtt_client, topic, message, strlen(message), 1, 0, mqtt_message_published_cb, (void *)topic);
}
#endif
